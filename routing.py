...
def main():
    ...
    # filename  for window ID dictionary
    ...
    print('Graph loaded!')
    if newLuTable:
        lookUpTable = trainNewLUTable(graphWithElevation, locationRequest, lutablefilenameFuel,windowIdDictFilename, mode="fuel")
    else:
        # load table from filename.pkl
        lookUpTable = LookUpTable(locationRequest, lutablefilenameFuel, windowIdDictFilename)
...
def trainNewLUTable(graphWithElevation, locationRequest, filename,windowIdDictFilename, mode='fuel'):
    ...
    del windowList
    gc.collect()
    lookUpTable = LookUpTable(locationRequest, filename, windowIdDictFilename, generateNewTable=True, parameterForTableIni=paramForTable)
    return lookUpTable
...
def plotRoutes(routeList, network_gdf, colorList, filename, nameLists = None):
    '''
    ...
    if nameLists is None:
        nameLists = ['path' for _ in range(len(routeList))]
    if len(colorList) == 1:
        color = colorList[0]
