...
class Pigat(nn.Module):
    ...
    def __init__(self, feature_dim, embedding_dim, num_heads, output_dimension,n2v_dim,window_size,attention_dim = 64):
        ...
        self.n2v.load_state_dict(torch.load('node2vec.mdl'))
...
class Pigat_withoutN2V(nn.Module):

    def __init__(self, feature_dim, embedding_dim, num_heads, output_dimension,n2v_dim,window_size,attention_dim = 64):
        super(Pigat_withoutN2V, self).__init__()
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        #self.n2v = N2V('node2vec.mdl')
        self.attention_dim = attention_dim
        self.embedding_dim = embedding_dim
...
#         segmentEmbed = torch.cat([self.n2v(id[:, i]).unsqueeze(1) for i in range(id.shape[1])], dim=1)
        ...
        # [ batch, window size, feature dimension+ sum embedding dimension + node2vec]
        x = torch.cat([x, embedded0,embedded1,embedded2,embedded3,embedded4,embedded5,embedded6], dim=-1)
        # [ window size, batch,  feature dimension+ sum embedding dimension]
        x = x.transpose(0, 1).contiguous()
        # q -> [1, batch, feature dimension+ sum embedding dimension]
        # middle of the window
...
class RecurPigat(nn.Module):

    def __init__(self, feature_dim, embedding_dim, num_heads, output_dimension,n2v_dim,window_size,hidden_dim = 0, attention_dim = 64):
        super(RecurPigat, self).__init__()
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        ...
        self.attention_dim = self.total_embed_dim
        self.selfattn = nn.MultiheadAttention(embed_dim= self.attention_dim, num_heads= self.num_heads)
        self.norm = LayerNorm(self.attention_dim+self.hidden_size )
        self.feed_forward = PositionwiseFeedForward(self.attention_dim+self.hidden_size)
        self.linear = nn.Linear(self.attention_dim+self.hidden_size,self.output_dimension)
        self.activate = nn.Softplus()
        self.middleOfTheWindow = window_size//2

    def forward(self, x, c, id, hidden_state):
        # x -> [ batch, window size, feature dimension]
        # c -> [batch, number of categorical features, window size]
        # hidden_state
        # [batch_sz, window_sz, embedding dim]
        ...
        # x_output -> [1, batchsz, feature dimension+ sum embedding dimension]
        x_output = q+x_output
        x_output = torch.cat((x_output.squeeze(0), hidden_state), 1)
        x_output = self.norm(x_output)
        x_output_ff = self.feed_forward(x_output)
        x_output = self.norm(x_output.squeeze(0) + x_output_ff)
        x_output = self.linear(x_output)
        # offset for velocity profile estimation to avoid 0 velocity
        # return F.relu(x_output)+0.1
        x_output = self.activate(x_output)
        return x_output, x_output[:,-self.hidden_size:]

    def init_hidden(self, batchsize):
        return nn.init.kaiming_uniform_(torch.empty(batchsize, self.hidden_size))

def testNet():
    # test nets
    net = RecurPigat(feature_dim=6, embedding_dim=[4, 2, 2, 2, 2, 4, 4], num_heads=1,
                  output_dimension=10, n2v_dim=32,window_size=3, hidden_dim=2)
    ...
    out, hidden_state = net(tmp,c,id, hidden_state)
    out, hidden_state = net(tmp, c, id, hidden_state)
